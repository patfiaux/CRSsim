...
RELICS_likelihood_calculation <- function(input.counts, input.par.list, input.specs){
  ...
  # total.guide.pos.ll <- rowSums(adjusted.guide.pos.ll)
  # total.guide.neg.ll <- rowSums(adjusted.guide.neg.ll)
  # guide.bayes.factor <- total.guide.pos.ll - total.guide.neg.ll
...
RELICS_search <- function(input.counts, input.info, input.specs){
  ...
  for(i in 1:length(input.specs$repl_groups)){
    ...
    temp.pr.auc.out <- precisionRecall_AUC(temp.bayes.factor, guide.type, 'pos', 'neg')
    repl.pars <- rbind(repl.pars, c(temp.par.list$out_df[1,], temp.auc.out$num_AUC,
      temp.pr.auc.out$num_AUC))
    ...
    colnames(repl.pars) <- c(names(temp.par.list$out_df[1,]), 'AUC', 'prAUC')
    ...
    if((temp.mean.auc - 2*temp.sd.auc) < 0.5){
      print('Warning: parameter estimates can not distinguish between positive and negative controls')
    }
